...
numBLS = 10
#numBLS = int(sys.argv) #varying with total number of personel being 24 (30)
numALS = 20 - (2*numBLS) #14 (16) (12)
...
def genCall() :
    ...
    if doInitialize:
        ...
        numOverCall, callHigh, callLow, location, nearStation, busyCall, serviceTime, nearStationNTV = genInitialCall(l)
    ...
    for c in range(numCall):
        ...
        nearStationNTV[c+numOverCall] = []
        for i in station:
            if dist[i,location[c+numOverCall]] < distThreshold:
                nearStation[c+numOverCall].append(i)
            if dist[i,location[c+numOverCall]] < distThresholdNVT:
                nearStationNTV[c+numOverCall].append(i)
    ...
    return numOverCall, numCall+numOverCall, callHigh, callLow, location, nearStation, busyCall, arrivalTime, serviceTime, nearStationNTV
...
def genInitialCall(l):
    ...
    for c1 in range(numOverCall):
        ...
        for i in station:
            ...
            if dist[i,location[c1]] < distThresholdNVT:
                nearStationNTV[c1].append(i)
        for i in station:
            busyCall[i,c1] = []
    ...
    return numOverCall, callHigh, callLow, location, nearStation, busyCall, serviceTime, nearStationNTV
