########################################
# SALT with BBC solution method for NVT with different Distance thresholds
...
import sample_Hanover_NTV_042720 as inputs #MINE
reload (inputs)
...
c_station = {}
for i in station : 
    c_station[i] = master.addConstr(quicksum(x[i,p] for p in serverType) <= 2) 
master.update()
...
def subproblem(opt,numOverCall, numCall, callHigh, callLow, location, nearStation, busyCall, nearStationNTV):
    ...
    for c in callLow : 
        ...
        c_coverLow[c] = sub.addConstr(z[c] - quicksum(y[i,'B',c] for i in nearStation[c]) <= 0)
    ...
    for c in callHigh : 
        ...
        #c_coverHigh1[c] = sub.addConstr(z[c] - quicksum(y[i,'A',c] for i in nearStation[c]) <= 0) #Change for al i later
        #Fnew# 
        c_coverHigh1[c] = sub.addConstr(z[c] - quicksum(y[i,'A',c] for i in nearStationNTV[c]) <= 0) #Change for al i later
    ...
    c_z2 = {}
    for c in callHigh:
        #K# c_zA[c] = sub.addConstr(zA[c] <= 1)
        #K# c_zB[c] = sub.addConstr(zB[c] <= 1)
        c_z2[c] = sub.addConstr(z[c] <= 1)
...
while cutfound and tottime < 2500:
    ...
    for s in scenario:
        ...
        subObjVal, rhs, xcoef = subproblem(opt, numOverCall[s], numCall[s], callHigh[s], callLow[s], location[s], nearStation[s], busyCall[s], nearStationNTV[s])
...
for i in station:
    for p in serverType:
        opt[i,p] = x[i,p].x
for s in scenario:
    v1,v2,v3 = subproblem(opt, numOverCall[s], numCall[s], callHigh[s], callLow[s], location[s], nearStation[s], busyCall[s],nearStationNTV[s])
    objEval_LP += v1
    objEval_IP += solveSSP_Int(opt, numOverCall[s], numCall[s], callHigh[s], callLow[s], location[s], nearStation[s], busyCall[s], nearStationNTV[s])
